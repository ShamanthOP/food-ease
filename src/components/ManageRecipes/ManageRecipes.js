...
const ManageRecipes = (props) => {
    const startRecipeNamesListener = useStoreActions(
        (actions) => actions.recipes.startRecipeNamesListener
    );
    const stopRecipeNamesListener = useStoreActions(
        (actions) => actions.recipes.stopRecipeNamesListener
    );
        ...
        <div id="manage-recipes-container">
            {loading ? (
                <div id="manage-recipes-container">
                    ...
                    {recipes.length === 0 && (
                        <div id="recipe-manager-add-recipe-container">
                            <h2 id="recipe-manager-no-recipes">
                                You have no recipes :(
                            </h2>
                            <Link
