...
const OrderedGroceries = (props) => {
    const startCategoryNameListener = useStoreActions(
        (actions) => actions.groceries.startCategoryNameListener
    );
    const stopCategoryNameListener = useStoreActions(
        (actions) => actions.groceries.stopCategoryNameListener
    );
    ...
    const startWeekGroceriesListener = useStoreActions(
        (actions) => actions.groceries.startWeekGroceriesListener
    );
    const stopWeekGroceriesListener = useStoreActions(
        (actions) => actions.groceries.stopWeekGroceriesListener
    );
    ...
    useEffect(() => {
        ...
        startWeekGroceriesListener({
            weekid: weekid,
            key: "ORDERED_GROCERIES",
        });
        return () => {
        ...
        <div className="ordered-groceries-container">
...

            {
                <div id="ordered-groceries-content">
                    ...
                    {activeHeader === "sorted" && (
                        <OrderedGroceriesTable sortedGroceries={localSorted} />
                    )}
                </div>
            }
        </div>
