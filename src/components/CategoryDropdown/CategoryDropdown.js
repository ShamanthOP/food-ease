...
function CategoryDropdown({ list, title, onChange }) {
    ...
    return (
        <div id="category-dd-wrapper">
            <div
                id={`category-dd-header_${listOpen ? "is-open" : ""}`}
                onClick={() => setListOpen(!listOpen)}
            >
                <div id="category-dd-header-title">
                    {title === "" ? "Pick a category" : list[title]}
                </div>
                {listOpen ? (
            ...
            {listOpen && (
                <ul id="category-dd-list" style={{ marginTop: "0px" }}>
                    {Object.keys(list).map((key) => {
                        return (
                            <li
                                key={key}
                                id="category-dd-list-item"
