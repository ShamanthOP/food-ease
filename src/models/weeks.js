...
const weeksModel = {
    ...
    populateLatestWeek: thunk(async (actions, payload) => {
        ...
        if (weeks.val() === null) {
            ...
            const latestYearWeeks = await weekRef
                .orderByChild("year")
                .equalTo(latestYear)
                .once("value");
            ...
            var weekNrValues = [];
            for (let i in latestYearWeeks.val()) {
            ...
            for (var j in latestYearWeeks.val()) {
                if (latestYearWeeks.val()[j].weekNr === latestWeekNr) {
                    ...
                    for (let i = 0; i < weekObj.recipes.length; i++) {
                        if (weekObj.recipes[i].recipeID !== "") {
                            var recipe = await database
                                .ref(
                                    `users/${uid}/recipes/${weekObj.recipes[i].recipeID}`
                                )
                                .once("value");
                            weekObj.recipes[i]["recipeID"] = recipe.val();
                            weekObj.recipes[i]["recipeID"].recipeID =
                                recipe.key;
                        }
                    }
    ...
    previousWeek: thunk(async (actions, payload) => {
        const uid = store.getState().auth.uid;
        const weekRef = database.ref(`users/${uid}/weeks`);
        const previousWeek = await weekRef
            .orderByChild("week_year")
            .equalTo(`${payload.week.weekNr - 1}_${payload.week.year}`)
            .once("value");
        ...
        if (previousWeek.val() === null) {
            ...
            for (var j = 0; j < weekObj.recipes.length; j++) {
                if (weekObj.recipes[j].recipeID !== "") {
                    var recipe = await database
                        .ref(
                            `users/${uid}/recipes/${weekObj.recipes[j].recipeID}`
                        )
                        .once("value");
    ...
    startAddWeek: thunk(async (actions, payload) => {
        ...
        if (payload.year === undefined) {
            ...
            const latestWeek = await weekRef
                .orderByChild("year")
                .equalTo(payload.year)
                .once("value");
    ...
    startAddYear: thunk(async (actions, payload) => {
        ...
        if (payload.week.year === undefined) {
            ...
            const latestYears = await weekRef
                .orderByChild("year")
                .startAt(payload.week.year)
                .once("value");
    ...
    startUpdateWeek: thunk(async (actions, payload) => {
        ...
        switch (payload.type) {
            case "TOTAL":
                await database
                    .ref(`users/${uid}/weeks/${payload.id}`)
            ...
            case "RECIPE":
                ...
                await database
                    .ref(`users/${uid}/weeks/${payload.id}/recipes/${dayIndex}`)
                    .update({ recipeID: payload.recipeID });
                var recipe = await database
                    .ref(`users/${uid}/recipes/${payload.recipeID}`)
                    .once("value");
                var weekObject = store.getState().weeks.week;
                weekObject.recipes[dayIndex].recipeID = recipe.val();
                weekObject.recipes[dayIndex]["recipeID"].recipeID =
                    payload.recipeID;
                actions.setWeek(weekObject);
                break;
            default:
                break;
        }
    ...
    addGrocery: thunk(async (actions, payload) => {
        ...
        switch (payload.type) {
            case "PRODUCT_UPDATE":
                await database
                    .ref(
                        `users/${uid}/weeks/${payload.id}/groceries/${groceryID}`
                    )
                    .update({ product: payload.item });
