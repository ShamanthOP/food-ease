...
const groceriesModel = {
    ...
    startWeekGroceriesListener: thunk(async (actions, payload) => {
        ...
        if (payload.weekid === null) {
            ...
            weekid = await database
                .ref(`users/${uid}/yearWeekNumbers/${year}_${weekNr}`)
                .once("value");
        }

        if (weekid !== null) {
            var weekGroceriesRef = database.ref(
                `users/${uid}/weeks/${payload.weekid}/groceries`
            );
    ...
    setGroceries: action((state, payload) => {
        switch (payload.type) {
            ...
            case "SORTED":
                if (
                    payload.category &&
                    state.sortedGroceries[payload.category]
                ) {
                    state.sortedGroceries[payload.category].push(
                        payload.grocery
                    );
                }
                break;
    ...
    addGrocery: thunk(async (actions, payload) => {
        ...
        if (payload.type === "NEW_CATEGORY") {
            categoryid = uniqid();
            updates[`users/${uid}/groceryCategories/${categoryid}`] =
                payload.category;
        } else {
        ...
        updates[
            `users/${uid}/groceryCategoryList/${categoryid}/${product}`
        ] = true;
    ...
    setAddGrocery: action((state, payload) => {
        payload.unsortedGroceries.forEach((unsortedGrocery, index) => {
            if (
                Object.keys(unsortedGrocery)[0] ===
                Object.keys(payload.grocery)[0]
            ) {
                state.unsortedGroceries.splice(index, 1);

                if (payload.type) {
                    var groceryObj = {};
                    groceryObj[payload.category] = [
                        {
                            product: Object.values(payload.grocery)[0].product,
                            amount: Object.values(payload.grocery)[0].amount,
                            groceryid: Object.keys(payload.grocery)[0],
                        },
                    ];
                    state.sortedGroceries = {
                        ...payload.sortedGroceries,
                        ...groceryObj,
                    };
                } else {
                    state.sortedGroceries[payload.category].push({
